...
export class BulkUploadChemicalComponent  extends BaseComponent implements OnInit {
  ...
  constructor(private chemicalService: ChemicalService, private toastrService: ToastrService,) {
    super();
  }
  ...
  upload() {
    if (this.isUpload) {
      this.isLoading = true;
      this.chemicalService.bulkuUloadChemicals(this.formData).subscribe(data => {
        this.response = data;
  ...
  onDownloadReport(recordType) {
    let heading = [['Name', 'Unit', 'Category', 'Cas number','H bond acceptor','H bond donor',
     'Iupac name', 'InChIKey','Molecular weight','Molecular formula','Synonyms','Image']];
