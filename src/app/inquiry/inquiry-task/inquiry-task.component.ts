...
export class InquiryTaskComponent extends BaseComponent implements OnInit {
  ...
  getInquiryTasks() {
    this.sub$.sink = this.inquiryTaskService
      .getInquiryTasks(this.inquiryId)
  ...
  onDelete(id: string) {
    this.sub$.sink = this.commonDialogService
      ...
      .subscribe((isTrue: boolean) => {
        if (isTrue) {
          this.sub$.sink = this.inquiryTaskService.deleteInquiryActivity(id).subscribe(() => {
            this.getInquiryTasks();
          });
        }
      });
  ...
  onChangeStatus(inquiryTask: InquiryTask) {
    this.sub$.sink = this.commonDialogService
      .deleteConformationDialog(
        this.translationService.getValue('ARE_YOU_SURE_WANT_TO_CHANGE_STATUS'),
      )
      .subscribe((isTrue: boolean) => {
        if (isTrue) {
          inquiryTask.isOpen = !inquiryTask.isOpen;
          this.sub$.sink = this.inquiryTaskService
            .updateInquiryActivity(inquiryTask.id, inquiryTask)
            .subscribe(() => {
              this.getInquiryTasks();
            });
        }
